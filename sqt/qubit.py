...
class Qubit:
    def __init__(self, bloch_vector: List[float]):
        ...
        self.density_matrix = 0.5*(np.eye(2) + pauli_x*self.bloch_vector[0] + pauli_y*self.bloch_vector[1] + pauli_z*self.bloch_vector[2])
    ...
    def trajectory(self, time_start: float, time_end: float, dt: float, stochastic: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def evolve(self, current_time: float, dt: float, stochastic: bool = True) -> None:
        """
